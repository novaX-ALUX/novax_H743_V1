...
- **Current compass correction:** `ROTATION_NONE` plus `AP_RM3100_REVERSAL_MASK 4` (Z inversion), matching `hwdef.dat`. The older `ROTATION_YAW_90` description no longer describes this firmware. Follow the current board definition and verify the installed orientation; do not restore the old correction from this document.
...
| File | When |
|------|------|
| `AP-RTK_dual-v<ver>_with_bl.hex` | **First flash** — combined bootloader + app via ST-Link / SWD (PA13/PA14), e.g. STM32CubeProgrammer |
...
Use the matching prebuilt firmware. The Z-axis correction and node name are part of the board definition, but installation-specific GNSS/heading parameters still require verification; this is not a claim that no module parameters ever need configuration.
